...
public class AdaptiveTrackSelection extends BaseTrackSelection {
  ...
  private int determineIdealSelectedIndex(long nowMs) {
    ...
    //for show bandwith indicator, add by jf
    try{
      if(length>0){
        int bandwidthHealthLevel;
//        Log.e("bitrateEstimate",">>"+effectiveBitrate+","+getFormat(length-1).bitrate+","+getFormat(0).bitrate);
//    int highestBitrate=Math.max(getFormat(length-1).bitrate,getFormat(0).bitrate);
//    int lowestBitrate=Math.min(getFormat(length-1).bitrate,getFormat(0).bitrate);
        if (effectiveBitrate >= getFormat(0).bitrate) {
          bandwidthHealthLevel = DefaultBandwidthMeter.BANDWIDTH_LEVEL_HIGH;
        } else if (effectiveBitrate <= getFormat(length-1).bitrate) {
          bandwidthHealthLevel =  DefaultBandwidthMeter.BANDWIDTH_LEVEL_LOW;
        } else {
          bandwidthHealthLevel =  DefaultBandwidthMeter.BANDWIDTH_LEVEL_MEDIUM;
        }

        if (((DefaultBandwidthMeter)bandwidthMeter).getBandwidthHealthListener() != null)
          ((DefaultBandwidthMeter)bandwidthMeter).getBandwidthHealthListener().onBandwidthHealthUpdate(bandwidthHealthLevel);
      }
