...
public class ManifestFetcher<T> implements Loader.Callback {
  ...
  public interface EventListener {
    ...
    public void onManifestRefreshed(Object newManifest);

    public void onManifestError(IOException e);

    public void onSingleFetchCompleted(Object newManifest);
  ...
  private void notifyManifestRefreshed() {
    if (eventHandler != null && eventListener != null) {
      eventHandler.post(new Runnable()  {
        @Override
        public void run() {
          eventListener.onManifestRefreshed(getManifest());
        }
      });
    }
  }
  ...
  private void notifySingleFetchCompleted() {
    if (eventHandler != null && eventListener != null) {
      eventHandler.post(new Runnable() {
        @Override
        public void run() {
          eventListener.onSingleFetchCompleted(manifest);
        }
      });
    }
  }
